...
| ~~Wheel~~ **PAUSED** | `wheel_strategy.py` | cron disabled (manual only) | Sell cash-secured puts → covered calls if assigned; close at 50% profit. **Paused 2026-07-26**: negative expectancy *by construction* — the "stop loss at 100% of premium" buys the short put back at a realized loss the instant it doubles, which defeats the wheel (an ITM put is meant to be *assigned*, then recovered via covered calls). On TSLA's high vol the 10%-OTM puts breach constantly: $1,094 premium collected but −$473 net (PF 0.22) over 4 cycles. Code kept. Revive only after (1) replacing the premium stop with assignment/roll-for-credit, (2) wheeling a name you'd want to own (not a single high-vol name), and (3) a backtest showing positive expectancy. |
