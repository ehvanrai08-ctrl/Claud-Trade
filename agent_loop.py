...
def run_discovery():
    ...
    try:
        ...
        print(f"[AGENT LOOP] Discovery output was not valid JSON", flush=True)
        return None
...
def run_backtest(strategy_description, limit=5):
    ...
    try:
        ...
        print(f"[AGENT LOOP] Backtest output was not valid JSON", flush=True)
        return None
...
def run_optimizer(strategy_name, param_description):
    ...
    try:
        ...
        print(f"[AGENT LOOP] Optimizer output was not valid JSON", flush=True)
        return None
...
def build_report(discovery, backtest_results, optimizer_results):
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"*Report generated by agent loop.*")
