...
# Import the backtest library from research
try:
...
def fetch_cache():
    """Load the existing cache of top-20 results."""
    cache_file = f"{BASE_DIR}/.efficient_discovery_cache.json"
    ...
    return {
        "tested_variations": {},
        "proven_families": {
            "tsmom_family": {"best": None, "sharpe_min": 0.9},
            "credit_vol_qqq": {"best": None, "sharpe_min": 1.0},
            "mean_reversion": {"best": None, "sharpe_min": 0.8},
            "golden_cross": {"best": None, "sharpe_min": 0.85},
        }
    }
...
def execute_backtest(code, name):
    ...
    try:
        ...
        try:
            ...
            return {"strategy": name, "error": f"No JSON output"}
    finally:
        try:
            os.unlink(temp_path)
        except:
...
def build_report(results):
    ...
    if tier1:
        ...
        for r in sorted(tier1, key=lambda x: x["sharpe"], reverse=True):
            lines.append(
                ...
                f"{r.get('cagr', '?')}% | {r.get('maxdd', '?'):.1f}% | {r.get('corr_spy', '?'):.2f} | ✓ Deploy |"
            )
    ...
    lines.extend([
        ...
        f"- **Method**: Systematic parameter grid on proven families",
        f"- **Recommendation**: Deploy Tier 1 candidates; backtest Tier 2 on live Alpaca data",
        "",
        "---",
