...
def fetch_resolved_markets(limit: int = 50, category: str | None = None) -> list[dict]:
    """Fetch recently settled markets from the Kalshi API."""
    path = f"{config.KALSHI_API_PREFIX}/markets"
    try:
        resp = httpx.get(
            f"{config.KALSHI_HOST}/markets",
            params={"limit": min(limit * 4, 1000), "status": "settled"},
        ...
        console.print(f"[red]Error fetching settled markets: {e}[/red]")
        return []
    ...
    for m in items:
        try:
            ...
            if result not in ("yes", "no"):
                continue
            resolved_yes_price = 1.0 if result == "yes" else 0.0
            ...
            if category:
                cat = _infer_category(question, [m.get("category", "")])
                if cat != category:
                    continue
