...
def confirm_fill(symbol, notional, est_price):
    ...
    must reflect the real position. Estimate is the fallback if the fill isn't
    visible yet."""
    time.sleep(2)
...
def run():
    ...
    for sym in list(held):
        ...
        if not pos:
            ...
            if qty > 0 and entry:
                record_trade("emerging_growth", sym, (px - entry) * qty,
        ...
        if order:
            ...
            print(f"[EMERGING] SELL {qty:.4f} {sym} | P&L ${pnl:+.2f}")
            held.pop(sym, None)
        else:
    ...
    if complete:
        ...
        print("[EMERGING] Rebalance incomplete — will retry next run")
